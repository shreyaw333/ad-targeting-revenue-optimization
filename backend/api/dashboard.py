...
def load_data():
    campaigns = pd.read_csv(DATA_PATH / "campaign_performance.csv")
    events = pd.read_csv(DATA_PATH / "user_events.csv")
    users = pd.read_csv(DATA_PATH / "user_features.csv")
    ads = pd.read_csv(DATA_PATH / "ad_features.csv")
...
def get_real_overview_data():
    ...
    avg_ctr = float(campaigns["ctr"].mean())
    ...
    for m in month_order:
        if m in monthly.index:
            ...
            revenue_data.append({
                "name": m,
                "revenue": round(rev),
                "ctr": round(avg_ctr + np.random.uniform(-0.3, 0.3), 2),
    ...
    prev_rev = campaigns["revenue"].sum() * 0.72  # simulate prior period
    ...
    return {
        "kpis": {
            ...
            "averageCTR":    {"value": round(avg_ctr, 2),          "change": round((avg_ctr - 2.6) / 2.6 * 100, 1)},
            "impressions":   {"value": total_impressions,          "change": 18.6},
        },
        "revenueData": revenue_data,
        "audienceData": audience_data,
        "performanceData": [
            {"metric": "CTR Improvement",    "value": f"+{round((avg_ctr - 2.6) / 2.6 * 100)}%", "trend": "up"},
            {"metric": "Revenue per User",   "value": f"+{round(rev_change)}%",                  "trend": "up"},
